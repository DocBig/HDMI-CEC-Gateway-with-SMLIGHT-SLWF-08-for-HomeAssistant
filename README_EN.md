...
**Enter your Wi-Fi credentials and OTA password:**  
...
**Set the HDMI port:**  
Open `esphome/hdmi-cec-gateway.yaml` and set the HDMI port you're plugging the SLWF-08 into:

```yaml
hdmi_cec:
  physical_address: 0x3000  # Adjust port here:
                             # 0x1000 = HDMI 1
                             # 0x2000 = HDMI 2
                             # 0x3000 = HDMI 3
                             # 0x4000 = HDMI 4
...
You also need to update the same value in the `0x83` handler's first byte:
...
The required HDMI-CEC component is automatically downloaded from GitHub during the build – no manual installation needed:
...
Replace all occurrences of `4dbde0` in `cec.yaml` with your own MAC suffix:
...
This is the only place you need to adapt for your device.
...
This project uses the [esphome-native-hdmi-cec](https://github.com/DocBig/esphome-native-hdmi-cec) component.
